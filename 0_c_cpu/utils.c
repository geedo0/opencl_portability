...
void check_results(float *host, float *device, int N) {
  int ii, jj;
  int error_count = 0;
  double error, error_normalized;
  printf("\nChecking results using tolerance %f\n", TOL);
  for(ii=0; ii<N; ii++) {
    for(jj=0; jj<N; jj++) {
      error = abs(host[ii*N + jj] - device[ii*N + jj]);
      error_normalized = error / (double) MAX_VALUE;
      if(error_normalized > TOL) {
        printf("Mismatch at %d,%d: host %.1f device %.1f\n",
